...
for name, activation_function in zip(["tanh", "sigmoid", "softmax", "relu"], [tf.nn.tanh, tf.nn.sigmoid, tf.nn.softmax, tf.nn.relu]):
    ...
    model = PINN(layers, X_u, Y_u, X_r, Y_r, activation_function=tf.nn.sigmoid)    
